...
# Clean descriptions
income_df["description_clean"] = income_df["description"].fillna("").astype(str).str.strip().str.upper()
...
with st.container():
    ...
    if not income_df.empty:
        min_date = income_df["date"].min()
        max_date = income_df["date"].max()
    else:
        min_date = date.today()
        max_date = date.today()
    ...
    # 2. Category Filter
    unique_categories = sorted(income_df["category"].dropna().astype(str).unique().tolist())
    with f_col2:
    ...
    with f_col3:
        search_term = st.text_input("Search Description (e.g., 'Bonus')", "")
...
with c2:
    ...
    st.caption("Identify specific income streams.")
    ...
    if not filtered_df.empty:
        ...
        sort_mode = st.radio("Sort By:", ["Total Amount ($)", "Frequency (Count)"], horizontal=True)

        if sort_mode == "Frequency (Count)":
            top_desc = desc_group.sort_values("Count", ascending=False).head(10)
            y_axis = "Count"
            title = "Most Frequent Income Sources"
        else:
